...
show_stats() {
    ...
    if [ -f improve_baseline_model.ipynb ]; then
        local file_size=$(du -h improve_baseline_model.ipynb | cut -f1)
        local backup_count=$(ls backups/*.ipynb 2>/dev/null | wc -l)
        local git_commits=$(git log --oneline | wc -l)
        local lines_of_code=$(jupyter nbconvert --to script --stdout improve_baseline_model.ipynb 2>/dev/null | wc -l)
        
        echo "📁 Размер основного файла: $file_size"
        echo "💾 Количество backup'ов: $backup_count"
        echo "📝 Git коммитов: $git_commits"
        echo "📊 Строк кода: $lines_of_code"
        echo "🕐 Последнее изменение: $(stat -c %y improve_baseline_model.ipynb)"
    else
        print_warning "Основной файл не найден"
    fi
}
